...
<div class="min-h-full">
    ...
    <main>
      <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        ...
        <!-- Create State Form -->
        <form id="createForm" method="POST" action="{{ url('state') }}">
            @csrf

            <label for="name">State</label>
            <input type="text" id="name" name="name">

            <button type="button" onclick="submitForm('createForm')" class="mt-2 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Submit</button>
        </form>
                ...
                <table class="min-w-full bg-white shadow-md rounded-xl">
                    ...
                    <tbody class="text-blue-gray-900">
                        ...
                        <tr class="border-b border-blue-gray-200">
                            <td class="py-3 px-4">{{ $state->id }}</td>
                            <td class="py-3 px-4">{{ $state->state }}</td>
                            <td class="py-3 px-4">
                                <a href="{{ url('state/' . $state->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a>
                                <form action="{{ route('state.destroy', ['id' => $state->id]) }}" method="POST" class="inline">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="font-medium text-red-600 hover:text-red-800 ml-2">Delete</button>
                                </form>
                                <a href="{{ url('cities/' . $state->id) }}" class="font-medium text-blue-600 hover:text-blue-800 ml-2">Show Cities</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
